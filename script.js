...
function displayHero(index) {
    ...
    // 5. Hero description More button
    const heroMoreBtn = document.getElementById('hero-more-btn');
    const heroDescEl = document.getElementById('hero-desc');
    if (heroMoreBtn && heroDescEl) {
        // Reset collapsed state on slide change
        heroDescEl.style.height = '';
        heroDescEl.style.overflow = 'hidden';
        heroDescEl.style.display = '-webkit-box';
        heroDescEl.style.webkitLineClamp = '3';
        heroDescEl.style.webkitBoxOrient = 'vertical';
        heroMoreBtn.style.display = 'none';
        heroMoreBtn.textContent = 'More \u25be';

        requestAnimationFrame(() => {
            if (heroDescEl.scrollHeight > heroDescEl.clientHeight + 2) {
                heroMoreBtn.style.display = 'block';
            }
        });
    }

    // 6. Hero Trailer Button — fetch videos in background
    const heroTrailerBtn = document.getElementById('hero-trailer-btn');
    if (heroTrailerBtn) {
        heroTrailerBtn.style.display = 'none';
        trailerKey = null; // Reset on slide change

        fetch(`${CONFIG.BASE_URL}/${item.media_type}/${item.id}/videos?api_key=${CONFIG.API_KEY}`)
            .then(r => r.json())
            .then(vdata => {
                const trailers = (vdata.results || []).filter(v => v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser'));
                const trailer = trailers.find(v => v.type === 'Trailer') || trailers[0];
                if (trailer) {
                    trailerKey = trailer.key;
                    heroTrailerBtn.style.display = 'flex';
                }
            })
            .catch(() => { });
    }
...
async function loadDetailsPage() {
    ...
    try {
        const res = await fetch(`${CONFIG.BASE_URL}/${type}/${id}?api_key=${CONFIG.API_KEY}&append_to_response=credits,recommendations,similar,external_ids,videos`);
        const data = await res.json();
        ...
        setupDescriptionToggle(data.overview);

        // Setup Trailer
        setupTrailer(data.videos);
...
function setupTrailer(videos) {
    ...
    const trailers = videos.results.filter(v => v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser'));
    const trailer = trailers.find(v => v.type === 'Trailer') || trailers[0];
...
window.handleTrailerBackdropClick = function (e) {
    if (e.target === document.getElementById('trailer-modal')) {
...
window.toggleHeroDesc = function () {
    ...
    const textSpan = document.getElementById('hero-more-text');
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') window.closeTrailerModal();
});

// --- DESCRIPTION TOGGLE ---
function setupDescriptionToggle(overview) {
    const el = document.getElementById('details-overview');
    const btn = document.getElementById('overview-more-btn');
    if (!el || !btn || !overview) return;

    el.innerText = overview;

    // Show the More button only if text is actually clamped
    requestAnimationFrame(() => {
...
window.toggleOverview = function () {
    ...
    const isExpanded = el.classList.contains('overview-expanded');
    if (isExpanded) {
        el.classList.remove('overview-expanded');
        el.classList.add('overview-clamp');
        btn.textContent = 'More ▾';
    } else {
        el.classList.remove('overview-clamp');
        el.classList.add('overview-expanded');
